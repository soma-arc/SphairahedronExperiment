...
void main() {
    ...
    for(float i = 0.; i < MAX_SAMPLES; i++){
        ...
        if (distance(position, u_zbzc) < u_ui.x) {
            // point (zb, zc)
            sum += vec3(1, 0, 0);
        } else if (abs(-2. * sqrt(3.) * position.x * position.y + 54.0 - 30.0 * sqrt(3.)) < u_ui.y) {
            // -2 * sqrt(3) zb zc + 54 - 30 * sqrt(3)
            sum += vec3(0, 1, 0);
        } else if (abs(-3. * position.x * position.x + 4. * position.x * position.y + 15.0 / 4.0) < u_ui.y) {
            // -3 zb^2 + 4 * zb * zc + 15 / 4
            sum += vec3(0, 1, 0);
        } else if (abs(2. * position.x * position.y -3. * position.y * position.y + (3. / 4.)) < u_ui.y) {
            // 2 zb zc - 3 * zc^2 + 3 / 4
            sum += vec3(0, 1, 0);
            ...
            sum += vec3(0.7);
                  ...
                  (-2. * sqrt(3.) * position.x * position.y + 54.0 - 30.0 * sqrt(3.)) > 0. &&
                  (-3. * position.x * position.x + 4. * position.x * position.y + 15.0 / 4.0) > 0. &&
                  (2. * position.x * position.y -3. * position.y * position.y + (3. / 4.)) > 0.) {
            sum += vec3(0.3, 0.3, 0.);
        }
    }
