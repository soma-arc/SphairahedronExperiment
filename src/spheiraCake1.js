...
export default class SpheiraCake1 {
    /**
     *
     * @param {number} zb
     */
    ...
    computeSpheres() {
        const r5 = Math.sqrt(6) / 3;
        //        const r6 = (2 + this.zb * this.zb) / Math.sqrt(6) - Math.sqrt(2) / 3;
        const r6 = r5;
//        const r6 = Math.sqrt(2);
        this.s5 = new Sphere(1 - (r5 / 4) * (Math.sqrt(6) + Math.sqrt(2)), 0, r5 / 4 * (Math.sqrt(6) - Math.sqrt(2)), r5);
        this.s6 = new Sphere((r6 / 4) * (Math.sqrt(6) + Math.sqrt(2)) - 1, this.zb, -r6 / 4 * (Math.sqrt(6) - Math.sqrt(2)), r6);
//        this.s6 = new Sphere(r6 / 4 * (Math.sqrt(6) - Math.sqrt(2)), this.zb, r6 / 4 * (Math.sqrt(6) + Math.sqrt(2)) - 1, r6);
    }

    computeInvSpheres() {
        this.s1inv = this.inversionSphere.invertOnPlane(new Vec3(1, 5, 0),
                                                        new Vec3(0, 1, 1),
                                                        new Vec3(0.5, 2, 0.5));
        this.s2inv = this.inversionSphere.invertOnPlane(new Vec3(0, 3, 1),
                                                        new Vec3(-1, 5, 0),
                                                        new Vec3(-0.5, 2, 0.5));
        this.s3inv = this.inversionSphere.invertOnPlane(new Vec3(-1, 3, 0),
                                                        new Vec3(0, 6, -1),
                                                        new Vec3(-0.5, 2, -0.5));
        this.s4inv = this.inversionSphere.invertOnPlane(new Vec3(1, 0, 0),
                                                        new Vec3(0, 3, -1),
                                                        new Vec3(0.5, 2, -0.5));
        this.s5inv = this.inversionSphere.invertOnSphere(this.s5);
        this.s6inv = this.inversionSphere.invertOnSphere(this.s6);
        console.log(this.s1inv);
        console.log(this.s2inv);
        console.log(this.s3inv);
        console.log(this.s4inv);
        console.log(this.s5inv);
        console.log(this.s6inv);
    }
    ...
    setUniformValues(gl, uniLocations, uniI, scale) {
        ...
        gl.uniform3f(uniLocations[uniI++],
                     this.s5.center.x, this.s5.center.y, this.s5.center.z);
        gl.uniform2f(uniLocations[uniI++],
                     this.s5.r, this.s5.rSq);
        gl.uniform3f(uniLocations[uniI++],
    ...
    computeSeedSpheres() {
        ...
        this.addSphereIfNotExists(this.seedSpheres,
                                  this.computeMinSeedSphere(this.vertexes[0], this.vertexes,
                                                            this.s1inv, this.s2inv, this.s5inv));
        this.addSphereIfNotExists(this.seedSpheres,
        ...
        this.addSphereIfNotExists(this.seedSpheres,
                                  this.computeMinSeedSphere(this.vertexes[2], this.vertexes,
                                                            this.s2inv, this.s5inv, this.s6inv));
        this.addSphereIfNotExists(this.seedSpheres,
        ...
        this.addSphereIfNotExists(this.seedSpheres,
                                  this.computeMinSeedSphere(this.vertexes[4], this.vertexes,
                                                            this.s3inv, this.s4inv, this.s6inv));
        this.addSphereIfNotExists(this.seedSpheres,
    ...
    computeVertexes() {
        this.vertexes = [];
        this.vertexes.push(this.computeIdealVertex(this.s1inv, this.s2inv, this.s5inv));
        this.vertexes.push(this.computeIdealVertex(this.s1inv, this.s4inv, this.s5inv));
        this.vertexes.push(this.computeIdealVertex(this.s2inv, this.s5inv, this.s6inv));
        this.vertexes.push(this.computeIdealVertex(this.s2inv, this.s3inv, this.s6inv));
        this.vertexes.push(this.computeIdealVertex(this.s3inv, this.s4inv, this.s6inv));
    ...
    select(mouse, scale) {
        ...
        if (Vec2.distance(mouse, new Vec2(this.zb, this.zc)) < 0.1) {
            this.selectedComponentId = SpheiraCake1.POINT_ZB_ZC;
    ...
    move(mouse) {
        if (this.selectedComponentId === SpheiraCake1.POINT_ZB_ZC) {
